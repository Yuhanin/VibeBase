...
export async function POST(request: Request) {
  ...
  const base = project.github_default_branch || 'main'
  const branch = (body.branch || `vibebase/task-${String(task.id).slice(0,8)}`).replace(/[^A-Za-z0-9._/-]/g,'-')
  const ref = await fetch(`https://api.github.com/repos/${project.github_owner}/${project.github_repo}/git/ref/heads/${base}`, { headers, cache:'no-store' })
  ...
  return NextResponse.json({ branch, base, taskId:task.id, reused:false })
}
