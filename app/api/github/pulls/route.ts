...
export async function GET(request: Request) {
  ...
  try {
    ...
    const data = await projectGithubGet(config, '/pulls?state=open&per_page=30')
    return NextResponse.json({ pullRequests:data.map((item:any)=>({ number:item.number, title:item.title, state:item.state, url:item.html_url, branch:item.head?.ref, base:item.base?.ref })) })
  } catch (error) {
